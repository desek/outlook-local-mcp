...
* **docs:** in-server documentation surface for LLM self-troubleshooting (CR-0061)
  - Embeds README, QUICKSTART, and Troubleshooting Guide into the binary via `embed.FS`
  - Exposes each document as an MCP resource at `doc://outlook-local-mcp/{slug}` (`resources/list` and `resources/read`)
  - Adds three verbs to the `system` aggregate tool: `list_docs`, `search_docs`, `get_docs`
  - `system.status` now includes a `docs` section with `base_uri`, `troubleshooting_slug`, and `version`
  - Known Graph error classes (`TooManyRequests`, `ErrorInvalidRequest`, auth errors) carry a `see` field pointing to the relevant troubleshooting anchor
  - Authors `docs/troubleshooting.md` covering authentication failures, token refresh, Keychain issues, Graph 429 throttling, InefficientFilter, mail flags, read-only mode, log locations, and account lifecycle
  - Adds `/llms.txt` at repository root following the AnswerDotAI llms.txt standard; generated by `make docs-bundle`
  - Refactors README.md to a human-audience front page; deep content lives in QUICKSTART.md, docs/troubleshooting.md, and docs/reference/outlook-local-mcp-spec.md
