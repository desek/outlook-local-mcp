...
As of v0.6.0 (CR-0060), all operations are grouped into four domain tools dispatched by an `operation` verb. The old individual tool names (`calendar_create_event`, `mail_list_folders`, etc.) are replaced by:

```
{tool: "calendar", args: {operation: "create_event", subject: "Team sync", ...}}
{tool: "mail",     args: {operation: "list_folders"}}
{tool: "account",  args: {operation: "list"}}
{tool: "system",   args: {operation: "status"}}
```

Call any domain tool with `operation: "help"` to get full documentation for every supported operation:

```
{tool: "calendar", args: {operation: "help"}}
...
- **4 aggregate MCP domain tools** replacing the former per-operation surface: `calendar` (15 verbs: list_calendars, list_events, get_event, search_events, create_event, update_event, delete_event, respond_event, reschedule_event, create_meeting, update_meeting, cancel_meeting, reschedule_meeting, get_free_busy, plus help), `mail` (up to 13 verbs depending on feature flags), `account` (7 verbs: add, remove, list, login, logout, refresh, plus help), `system` (help, status, and optionally complete_auth). Each domain tool exposes an `operation="help"` verb returning detailed per-verb documentation. Cold-start tool-schema payload is 92% smaller than the pre-v0.6.0 surface (see CR-0060). Event verbs handle personal calendar entries; meeting verbs handle events with attendees and include confirmation guidance (see CR-0054)
...
- **Event provenance tagging** -- every event created by the MCP server is tagged with a hidden extended property, enabling reliable identification and filtering of MCP-created events via `created_by_mcp` on the `search_events` verb of the `calendar` tool. Invisible in Outlook UI. Configurable via `OUTLOOK_MCP_PROVENANCE_TAG`; set to empty to disable (see CR-0040)
...
- **User confirmation for attendee actions** -- the `create_meeting`, `update_meeting`, `reschedule_meeting`, and `cancel_meeting` verbs under the `calendar` tool include unconditional confirmation guidance: the LLM must present a draft summary and wait for explicit user confirmation before proceeding. An additional warning is shown when external attendees (different email domain) are involved. When the `AskUserQuestion` tool is available, the LLM is instructed to use it for a structured confirmation experience. Event verbs (without attendees) require no confirmation and redirect to meeting verbs when attendees are needed (see CR-0053, CR-0054)
- **Mail read access** -- opt-in read-only email access via `OUTLOOK_MCP_MAIL_ENABLED=true`. The `mail` tool gains verbs `list_folders`, `list_messages`, `search_messages`, `get_message`, `get_conversation`, `list_attachments`, and `get_attachment`, enabling KQL full-text search, OData filtering (including `is_read`, `is_draft`, `has_attachments`, `importance`, `flag_status`, and `provenance`), conversation-thread retrieval, and attachment download (default limit 10 MB). Adds `Mail.Read` OAuth scope only when enabled. Default: disabled (see CR-0043, CR-0058)
- **Mail draft management** -- opt-in draft-centric workflow via `OUTLOOK_MCP_MAIL_MANAGE_ENABLED=true` (implies `MAIL_ENABLED`). The `mail` tool gains verbs `create_draft`, `create_reply_draft`, `create_forward_draft`, `update_draft`, and `delete_draft`, letting the model compose replies, forwards, and new drafts that appear in the user's Outlook Drafts folder for review and manual send. The model never sends email: `Mail.Send` is never requested. Adds `Mail.ReadWrite` OAuth scope (supersedes `Mail.Read`). MCP-created drafts are tagged with the provenance extended property when `OUTLOOK_MCP_PROVENANCE_TAG` is configured. Default: disabled (see CR-0058)
