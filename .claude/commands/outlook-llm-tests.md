...
- Appends one row of run configuration (model, thinking effort) and metrics (duration, turns, cost, token usage, cache hits) to `docs/bench/crud-runs.csv`.
...
Defaults: account `default`, model `claude-sonnet-5`, thinking effort `low`. Override via env vars: `ACCOUNT=foo MODEL=claude-opus-5 THINKING=medium make crud-test`. `THINKING` must be one of `low|medium|high|xhigh|max`.
