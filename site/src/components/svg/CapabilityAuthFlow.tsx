...
export default function CapabilityAuthFlow({ className }: Props) {
  ...
  return (
    ...
    >
      <defs>
        {/* Card shadow */}
        <filter id={`af-card-shadow-${uid}`} x="-10%" y="-10%" width="120%" height="130%">
          <feDropShadow dx="0" dy="3" stdDeviation="5" floodColor="#000000" floodOpacity="0.55" />
        </filter>
        {/* Lime glow for trigger / result boxes */}
        <filter id={`af-lime-glow-${uid}`} x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="0" stdDeviation="7" floodColor="#abff02" floodOpacity="0.3" />
        </filter>
        {/* Dot glow */}
        <filter id={`af-dot-glow-${uid}`} x="-100%" y="-100%" width="300%" height="300%">
          <feDropShadow dx="0" dy="0" stdDeviation="3" floodColor="#abff02" floodOpacity="0.9" />
        </filter>
        {/* Arrowhead marker — lime */}
        <marker id={`af-arrow-lime-${uid}`} markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
          <polygon points="0 0, 7 3.5, 0 7" fill="#abff02" fillOpacity="0.7" />
        </marker>
        {/* Arrowhead marker — gray */}
        <marker id={`af-arrow-gray-${uid}`} markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
          <polygon points="0 0, 7 3.5, 0 7" fill="#454742" />
        </marker>
        {/* Flow dot animation path — trigger to device-code */}
        <path id={`af-path-1-${uid}`} d="M 240 70 L 108 118" fill="none" />
        {/* Flow dot animation path — trigger to browser */}
        <path id={`af-path-2-${uid}`} d="M 240 70 L 240 118" fill="none" />
        {/* Flow dot animation path — trigger to auth-code */}
        <path id={`af-path-3-${uid}`} d="M 240 70 L 372 118" fill="none" />
        {/* Flow dot animation path — device-code to token */}
        <path id={`af-path-4-${uid}`} d="M 108 202 L 240 250" fill="none" />
        {/* Flow dot animation path — browser to token */}
        <path id={`af-path-5-${uid}`} d="M 240 202 L 240 250" fill="none" />
        {/* Flow dot animation path — auth-code to token */}
        <path id={`af-path-6-${uid}`} d="M 372 202 L 240 250" fill="none" />
      </defs>

      {/* ══════════════════════════════════════
          TRIGGER BOX — "First Tool Call"
          ══════════════════════════════════════ */}
      <g filter={`url(#af-lime-glow-${uid})`}>
      ...
      <line x1="200" y1="72" x2="108" y2="118"
        ...
        markerEnd={`url(#af-arrow-lime-${uid})`} />
      {/* Arrow to Browser */}
      ...
      <circle r="3.5" fill="#abff02" filter={`url(#af-dot-glow-${uid})`}>
      ...
      {/* ══════════════════════════════════════
          ...
          ══════════════════════════════════════ */}
      <g filter={`url(#af-card-shadow-${uid})`}>
      ...
      <g transform="translate(326, 142)">
        ...
        <line x1="-7" y1="2" x2="2" y2="2" stroke="#abff02" strokeWidth="0.8" strokeOpacity="0.7" markerEnd={`url(#af-arrow-lime-${uid})`} />
        <line x1="7" y1="5" x2="-2" y2="5" stroke="#7f7f7f" strokeWidth="0.8" strokeOpacity="0.7" markerEnd={`url(#af-arrow-gray-${uid})`} />
      </g>
      ...
      <circle r="3" fill="#abff02" filter={`url(#af-dot-glow-${uid})`}>
