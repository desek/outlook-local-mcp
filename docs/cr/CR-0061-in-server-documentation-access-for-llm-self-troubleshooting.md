...
- [ ] `README.md` refactored for a human audience and links to `llms.txt`
- [ ] `QUICKSTART.md` and `docs/reference/outlook-local-mcp-spec.md` absorb the deep content displaced from `README.md`
- [ ] `llms.txt` generated at repository root, structurally compliant with the standard, regenerated by `make docs-bundle`
- [ ] `README.md` and `QUICKSTART.md` reference the new `system.list_docs` / `search_docs` / `get_docs` verbs
- [ ] `extension/manifest.json` requires no change (aggregate tool count remains 4 per CR-0060)
- [ ] `docs/prompts/mcp-tool-crud-test.md` updated with steps for the three new system verbs
