...
// embed directive in docs/embed.go, this test fails the build.
func TestBundle_OnlyAllowedSlugsPresent(t *testing.T) {
	...
	err := fs.WalkDir(extdocs.Bundle, ".", func(path string, d fs.DirEntry, err error) error {
		...
		// Derive slug from path "{slug}.md" (no files/ prefix).
		if !strings.HasSuffix(path, ".md") {
			t.Errorf("unexpected file path in bundle: %q (expected *.md)", path)
			return nil
		}
